    ...
    <main class="grid grid-cols-4 gap-8 mt-8 wrapper">
        ...
        <section class="flex flex-col col-span-3 gap-y-4">
            <small class="text-sm text-gray-400">
                ...
                <a href="{{ route('threads.show', [$thread->category->slug(), $thread->slug()]) }}">{{$thread->title()}}</a> >
                Edit
            </small>
            ...
            <article class="p-5 bg-white shadow">
                <div class="grid grid-cols-8">
                    ...
                    <div class="col-span-7 space-y-6">
                        <x-form action="{{route('threads.update', $thread)}}" method="PUT">
                            <div class="space-y-8">
                                {{-- Title --}}
                                <div>
                                    <x-form.label for="title" value="{{ __('Title') }}" />
                                    <x-form.input id="title" class="block w-full mt-1" type="text" name="title" :value="old('title') ?? $thread->title" required autofocus />
                                     ...
                                     multiple x-data="{}" x-init="function(){choices($el)}">
                                        ...
                                        @foreach ($tags as $tag)
                                            <option value="{{$tag->id()}}" 
                                                ...
                                                @elseif(in_array($tag->id(), $oldTags)) selected
                                                @endif
                                            >
                                                {{$tag->name()}}
                                            </option>
                                    ...
                                    <select name="category" id="category" class="w-full text-sm border-gray-300 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50">
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{$category->id()}}"
                                                @if($category->id == old('category')) selected 
                                                @elseif($category->id == $thread->category->id) selected
                                                @endif 
                                            >
                                                {{$category->name()}}
                                            </option>
